 ...
 */
class HealthFrag : Fragment(), View.OnClickListener, ActionbarFragment.ClickInterface, NumberPicker.OnValueChangeListener {
            ...
            R.id.hiker -> {
                view?.let {
                    Navigation.findNavController(it)
                        .navigate(R.id.action_healthFrag_to_mapsFrag)
                }
            }
            R.id.weather -> {
                view?.let {
                    Navigation.findNavController(it)
                        .navigate(R.id.action_healthFrag_to_weatherFrag)
                }
            }
            R.id.settings -> {
                view?.let {
                    Navigation.findNavController(it)
                        .navigate(R.id.action_healthFrag_to_settingsFrag)
                }
            }
            R.id.home -> {
                view?.let {
                    Navigation.findNavController(it)
                        .navigate(R.id.action_healthFrag_to_homeScreenFrag)
                }
            }
        }
    }
    ...
    private fun updateGoals()
    {
        ...
        calorieReq = calculateDifference(userActivityLevelConstant?.let {
            calculateTDEE(
                calculateBMR(
                    user?.sex.toString(),
                    getYears(user?.age), getHeight(user?.height), getWeight(user?.weight)
                ), it
            )
